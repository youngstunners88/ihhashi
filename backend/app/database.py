...
async def ensure_indexes(db: AsyncIOMotorDatabase) -> List[str]:
    ...
    await db.users.create_index("verification_level")
    ...
    await db.referrals.create_index([("referrer_id", ASCENDING), ("status", ASCENDING)])
    results.append("referrals")
    
    # Customer reward accounts indexes
    ...
    results.append("customer_reward_accounts")
    
    # Coin transactions indexes
    await db.coin_transactions.create_index("customer_id")
    await db.coin_transactions.create_index([("customer_id", DESCENDING), ("created_at", DESCENDING)])
    await db.coin_transactions.create_index("transaction_type")
    results.append("coin_transactions")
    ...
    await db.reward_redemptions.create_index([("customer_id", DESCENDING), ("created_at", DESCENDING)])
    results.append("reward_redemptions")
    ...
    await db.merchants.create_index("is_active")
    await db.merchants.create_index([("name", TEXT), ("description", TEXT)])
    results.append("merchants")
    
    logger.info(f"Created indexes for {len(results)} collections")
    return results
...
__all__ = [
    # Connection
    "connect_db", "close_db", "health_check", "ensure_indexes", "database",
